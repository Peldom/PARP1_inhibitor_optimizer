...
def enum_file(input_file,output_file):
    ...
    with open(output_file, 'w') as g:
        with open(input_file, 'r') as f:
            for smis in f.readlines():
                LIST = [sme.randomize_smiles(smis.strip()) for i in range(1000)]
                for smi in set(LIST):
                    g.write(smi + '\n')

if __name__ == "__main__":
    out_file = input('请输入文件输出路径')
    input_file = input('请输入文件输入路径')
    enum_file(input_file,out_file)
