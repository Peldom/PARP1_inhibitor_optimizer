...
class ORGANIC(QtCore.QObject):
    ...
    def load_training_set(self, file):
        ...
        self.vo = vocab.create_vocab(file)
        # self.char_dict, self.ord_dict = mm.build_vocab(self.train_samples)
        self.char_dict, self.ord_dict = vocab.build_vocab(self.vo)
        ...
        for i in self.positive_samples:
            print(len(i))
    ...
    def define_metric_as_remap(self, name, metric, remapping):
        ...
        if self.verbose:
            print('Defined create_vocabmetric {}'.format(name))
    ...
    def pretrain(self):
        ...
        for epoch in tqdm(range(self.PRETRAIN_GEN_EPOCHS)):
            ...
            g_loss_list.append(loss)
            self.signal_draw.emit("pre_gen",{"pre_gen_loss":g_loss_list})


            if epoch % 10 == 0:
                print('\t train_loss {}'.format(loss))
        epochs = range(len(g_loss_list))
		
        plt.plot(epochs, g_loss_list, 'b', label='gen_loss')
        plt.title('GENERATOR PRETRAINING')
        plt.legend()  # 绘制图例，默认在右上角
        # plt.show()
        plt.savefig('./fig/{}_GEN_PRETRAINING.png'.format(self.PREFIX))
        samples = self.generate_samples(self.SAMPLE_NUM)
        samples_list=[]
        i=0
        for s in samples:
            s_decode=mm.decode(s, self.ord_dict)
            samples_list.append(s_decode)
            try:
                mol1 = Chem.MolFromSmiles(s_decode)
                mols = mol1
                img = Draw.MolsToImage([mols], subImgSize=(512, 512))
                img.save("./fig/molecular/{}.png".format(i))
                i += 1
        ...
        if self.LAMBDA != 0:
            ...
            for i in tqdm(range(self.PRETRAIN_DIS_EPOCHS)):
                ...
                for batch in dis_batches:
                    ...
                    # print(loss,accuracy)
                    if i<=1:
                        d_loss_list.append(loss)
                        d_acc_list.append(accuracy)
                    # d_loss_list.append(self.discriminator.loss)
                    # d_acc_list.append(self.discriminator.accuracy)
                if i % 10 == 0:
                    print('\t train_loss {}'.format(loss))
                print(d_loss_list,d_acc_list)
                batchs = range(len(d_acc_list))
                plt.plot(batchs, d_acc_list, 'r', label='dis_acc')  # 'bo'为画蓝色圆点，不连线
                plt.plot(batchs, d_loss_list, 'b', label='dis_loss')
                plt.title('DISCRIMINATOR PRETRAINING')
                plt.legend()  # 绘制图例，默认在右上角
                plt.savefig('./fig/{}_DIS_PRETRAINING.png'.format(self.PREFIX))
                # plt.show()
        self.PRETRAINED = True
    ...
    def train(self, ckpt_dir='checkpoints/'):
        ...
        for nbatch in tqdm(range(self.TOTAL_BATCH)):
            ...
            # results
            # mm.compute_results(
            #     gen_samples, self.ord_dict, results)
            #
            ...
            if self.LAMBDA != 0:
                ...
                d_loss_list = []
                d_acc_list = []
                for i in tqdm(range(self.DIS_EPOCHS)):
                    ...
                    for batch in dis_batches:
                        ...
                        self.signal_draw.emit("dis",{
							"dis_loss":d_loss_list,
							"dis_accuracy":d_acc_list})
                    if i % 10 == 0:
                        print('\t train_loss {}'.format(d_loss))
                    #print(d_loss_list,d_acc_list)
                batchs=range(len(d_acc_list))
                plt.plot(batchs, d_acc_list, 'r', label='dis_acc')  # 'bo'为画蓝色圆点，不连线
                plt.plot(batchs, d_loss_list, 'b', label='dis_loss')
                ...
                plt.savefig('./fig/{}_DIS_TRAINING.png'.format(self.PREFIX))
                plt.show()
                ...
                print('d_loss is {},accuracy is {}'.format(d_loss, accuracy))
...
# def circulate():
#
#     # with open('../data/trainingsets/success_4.smi',mode='a+',encoding='utf-8')as g:
#     #     with open('./epoch_data/metrics_0.smi',mode='r')as f:
#     #         for line in f:
#     #             g.write(line)
#     # os.remove('./epoch_data/metrics_0.smi')
#     with open('../data/trainingsets/Value_Ki.smi',mode='a+',encoding='utf-8')as h:
#         with open('success_4_.smi', mode='r', encoding='utf-8')as j:
#             for line in j:
#
#                 h.write(line)
...
if __name__ == '__main__':
    # for V in range(10):
    #     # Setup model
    #     print('++++++++++++++++第{}次循环++++++++++++++++'.format(V))
    tf.reset_default_graph()
    model = ORGANIC('metrics', params={
        'GEN_ITERATIONS': 1,
        'PRETRAIN_GEN_EPOCHS':10,
        'PRETRAIN_DIS_EPOCHS':5,
        'DIS_EPOCHS': 1,
        'DIS_BATCH_SIZE': 30,
        'GEN_BATCH_SIZE': 30,
        'GEN_EMB_DIM': 32,
    ...
    model.load_training_set('../data/trainingsets/Value_Ki.smi')
    model.set_training_program(['logP'], [10])
    model.load_metrics()
