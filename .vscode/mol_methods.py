...
def encode(smile, max_len, char_dict): 
    ...
    j = 0
    while j < len(s):
        if j < len(s) - 1 and s[j:j + 7] in char_dict:
            seq.append(char_dict[s[j:j + 7]])
            j = j + 7
        elif j < len(s) - 1 and s[j:j + 6] in char_dict:
            seq.append(char_dict[s[j:j + 6]])
            j = j + 6
        elif j < len(s) - 1 and s[j:j + 5] in char_dict:
            seq.append(char_dict[s[j:j + 5]])
            j = j + 5
        elif j < len(s) - 1 and s[j:j + 4] in char_dict:
            seq.append(char_dict[s[j:j + 4]])
            j = j + 4
        elif j < len(s) - 1 and s[j:j + 3] in char_dict:
            seq.append(char_dict[s[j:j + 3]])
            j = j + 3
        elif j < len(s) - 1 and s[j:j + 2] in char_dict:
            seq.append(char_dict[s[j:j + 2]])
            j = j + 2

        elif s[j] in char_dict:
            seq.append(char_dict[s[j]])
            j = j + 1
    # if len(seq[0])<max_len:
    for i in range (max_len - len(seq)):
        seq.append(char_dict['_'])
    return seq
...
def compute_results(model_samples, ord_dict, results={}, verbose=True):
    # samples=[]
    # i=0
    # for s in model_samples:
    #     s_decode=decode(s, ord_dict)
    #     samples.append(s_decode)
    #     try:
